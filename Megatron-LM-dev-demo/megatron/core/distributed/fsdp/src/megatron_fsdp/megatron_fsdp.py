...
class MegatronFSDP(torch.nn.Module):
    ...
    def _register_fsdp_hooks(self, root_module):
        ...
        def _grad_acc(param):
            ...
            """
            # with open("yhc_grad_debug.txt", "a") as f:
            #     # 打印更多信息来验证 fuse_wgrad_accumulation 是否生效
            #     grad_added = getattr(param, 'grad_added_to_main_grad', 'N/A')
            #     is_te_linear = 'weight' in self.param_to_name[param] and '.q.' in self.param_to_name[param]
            #     strs_to_write = f"HaochenYuan debug: param_name={self.param_to_name[param]}, param.grad is None: {param.grad is None}, grad_added_to_main_grad: {grad_added}"
            #     f.write(strs_to_write + "\n")
            ...
            # # 验证：函数末尾 param.grad 应该是 None（对于 grad_added_to_main_grad=True 的情况）
            # with open("yhc_grad_debug.txt", "a") as f:
            #     f.write(f"[AFTER] param_name={self.param_to_name[param]}, param.grad is None: {param.grad is None}\n")
